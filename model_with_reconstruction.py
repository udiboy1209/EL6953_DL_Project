...
class Decoder(nn.Module):
    ...
    def forward(self, x, data):
        ...
        masked = Variable(torch.sparse.torch.eye(7))
        masked = masked.cuda()
...
class FixCapsNet(nn.Module):
    def __init__(self,conv_inputs,conv_outputs,
        ...
        self.Convolution = make_features(cfgs[mode],
        f_c=conv_inputs,out_c=conv_outputs)
        ...
        #self.load_state_dict(torch.load('./best_HAM10000_0507_033609.pth', map_location='cuda'), strict=False)
        
        self.decoder = Decoder(299, 299, conv_inputs)
    ...
    def forward(self, x):
        ...
        reconstructions, masked = self.decoder(out, x)
    ...
    def reconstruction_loss(self, data, reconstructions):
        ...
        return loss * 0.0005
    #margin_loss           
    def loss(self, data, output, target, reconstructions, size_average=True):
        ...
        if size_average:
            L_c = torch.mean(L_c)
        return L_c + self.reconstruction_loss(data, reconstructions)
